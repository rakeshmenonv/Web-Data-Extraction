var detailview = $.extend({}, $.fn.datagrid.defaults.view, {
	render: function(target, container, frozen){
		...
		for(var i=0; i<rows.length; i++) {
			...
			table.push('<tr rowspan="2" id="' + rowId + '" datagrid-row-index="' + i + '" ' + cls + ' ' + style + '>');
			table.push(this.renderRow.call(this, target, fields, frozen, i, rows[i]));
			table.push('</tr>');
			
			table.push('<tr  style="display:none;">');
			if (frozen){
				table.push('<td r colspan=' + (fields.length+2) + ' style="border-right:0">');
			} else {
	...
	onBeforeRender: function(target){
		...
		if (!hasExpander){
			...
			var td = $('<td rowspan="1"><div class="datagrid-header-expander" style="width:24px;"></div></td>');
			if ($('tr',t).length == 0){
				td.wrap('<tr></tr>').parent().appendTo($('tbody',t));
			} else if (opts.rownumbers){
				t.find('td:has(div.datagrid-header-rownumber)').attr("rowspan","1");
