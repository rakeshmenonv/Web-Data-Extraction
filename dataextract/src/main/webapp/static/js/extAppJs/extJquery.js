var infotop = infotop || {};
infotop.data = infotop.data || {};// 用于存放临时的数据或者对象
...
/**
 * 增加命名空间功能
 * 使用方法：infotop.ns('jQuery.bbb.ccc','jQuery.eee.fff');
 */
infotop.ns = function() {
	var o = {}, d;
...
/**
 * 将form表单元素的值序列化成对象
 * @example infotop.serializeObject($('#formId'))
 * @requires jQuery
 * @returns object
 */
infotop.serializeObject = function(form) {
	var o = {};
...
/**
 * 增加formatString功能
 * @example infotop.formatString('字符串{0}字符串{1}字符串','第一个变量','第二个变量');
 * @returns 格式化后的字符串
 */
 ...
 * @returns list
 */
infotop.stringToList = function(value) {
 ...
 * @returns
 */
infotop.jsonToString = function(o) {
	var r = [];
	if (typeof o == "string")
	...
	if (typeof o == "object") {
		if (!o.sort) {
			for ( var i in o)
				r.push(i + ":" + infotop.jsonToString(o[i]));
			...
			for ( var i = 0; i < o.length; i++)
				r.push(infotop.jsonToString(o[i]));
			r = "[" + r.join() + "]";
		}
		return r;
	}
 ...
 * 
 * @example infotop.cookie('the_cookie', 'the_value');
 * @desc Set the value of a cookie.
 * @example infotop.cookie('the_cookie', 'the_value', { expires: 7, path: '/', domain: 'jquery.com', secure: true });
 * @desc Create a cookie with all available options.
 * @example infotop.cookie('the_cookie', 'the_value');
 * @desc Create a session cookie.
 * @example infotop.cookie('the_cookie', null);
 ...
 * 
 * @example infotop.cookie('the_cookie');
 ...
 * @author Klaus Hartl/klaus.hartl@stilbuero.de
 */
infotop.cookie = function(key, value, options) {
 ...
 */
$(function() {
	$('.iconImg').attr('src', infotop.pixel_0);
});
