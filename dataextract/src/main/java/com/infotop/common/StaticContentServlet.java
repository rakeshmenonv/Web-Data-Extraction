 ...
 * 
 * 演示访问地址为： static-content?contentPath=static/images/logo.jpg
...
public class StaticContentServlet extends HttpServlet {
	...
	/** 需要被Gzip压缩的Mime类型. */
	private static final String[] GZIP_MIME_TYPES = { "text/html",
...
//	private ApplicationContext applicationContext;
	...
	protected void doGet(HttpServletRequest request,
		...
		// 取得参数
		String contentPath = getQueryParam(request, "contentPath");
		if (StringUtils.isBlank(contentPath)) {
			response.sendError(HttpServletResponse.SC_BAD_REQUEST,
					"contentPath parameter is required.");
			return;
		}
...
//				.getWebApplicationContext(getServletContext());
	...
	private ContentInfo getContentInfo(String contentPath) {
		...
		contentInfo.mimeType = mimetypesFileTypeMap
